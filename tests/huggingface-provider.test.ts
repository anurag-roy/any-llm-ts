...
function fakeClients(inference: Partial<HuggingFaceInferenceClientLike>, responsesCreate = vi.fn()): HuggingFaceProviderClients {
  return {
    inference: inference as HuggingFaceInferenceClientLike,
    responses: { responses: { create: responsesCreate } } as unknown as OpenAI,
  };
}
...
describe("Hugging Face provider", () => {
  ...
  it("uses the official inference client and normalizes empty tool arguments", async () => {
      ...
      choices: [
        {
          ...
          message: {
            content: null,
            role: "assistant",
            tool_calls: [{ function: { arguments: null, name: "now" }, id: "call-1", type: "function" }],
          },
        },
      ],
      created: 1,
    ...
    await expect(provider.completion({
      ...
      choices: [{ message: { toolCalls: [{ function: { arguments: "{}" }, id: "call-1" }] } }],
  ...
  it("normalizes official inference streaming chunks", async () => {
    async function* chunks(): AsyncIterable<Record<string, unknown>> {
      yield {
        choices: [{ delta: { content: "Hi", role: "assistant" }, finish_reason: null, index: 0 }],
        created: 1,
        id: "chunk-1",
        model: "org/model",
      };
    ...
    const provider = new HuggingFaceProvider(
      { apiKey: "hf-token" },
      fakeClients({ chatCompletion: vi.fn(), chatCompletionStream: vi.fn().mockReturnValue(chunks()) }),
    );
    const stream = await provider.completion({ messages: [], model: "org/model", stream: true });
    const values: ChatCompletionChunk[] = [];
    for await (const chunk of stream as AsyncIterable<ChatCompletionChunk>) values.push(chunk);
    expect(values).toMatchObject([{ choices: [{ delta: { content: "Hi" } }], provider: "huggingface" }]);
  });
  ...
  it("uses the separate OpenResponses router", async () => {
    ...
    expect(create).toHaveBeenCalledWith(expect.objectContaining({ input: "hello", model: "org/model" }));
  });
