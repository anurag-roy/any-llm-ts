...
function jsonBody(value: JsonValue | undefined): Uint8Array {
  return new TextEncoder().encode(JSON.stringify(value));
}

function fakeClient(send: ReturnType<typeof vi.fn>): SageMakerRuntimeClientLike {
...
describe("SageMaker provider", () => {
  ...
  it.each([
    ...
    const provider = new SageMakerProvider(
      {},
      fakeClient(vi.fn().mockResolvedValue({ Body: jsonBody(body) })),
    );
  ...
  it("normalizes event streams and skips non-payload or malformed events", async () => {
    async function* body(): AsyncIterable<{ PayloadPart?: { Bytes?: Uint8Array } }> {
      yield {};
    ...
    expect(send.mock.calls[0]?.[0]).toBeInstanceOf(InvokeEndpointWithResponseStreamCommand);
  ...
  it("rejects unsupported parameters, inputs, responses, and operations", async () => {
    ...
    const invalid = new SageMakerProvider(
      {},
      fakeClient(vi.fn().mockResolvedValue({ Body: jsonBody({ embedding: "invalid" }) })),
    );
    await expect(invalid.embedding({ input: "hello", model: "endpoint" })).rejects.toThrow(
      /numeric vector/u,
    );
