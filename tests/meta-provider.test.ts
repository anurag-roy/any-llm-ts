...
function clients(messageCreate: ReturnType<typeof vi.fn>, openAICreate = vi.fn()): {
  ...
  return {
    anthropic: { messages: { create: messageCreate } } as unknown as Anthropic,
    openai: {
...
describe("Meta provider", () => {
  ...
  it("uses Meta's native Anthropic-compatible Messages route", async () => {
    ...
    expect(create).toHaveBeenCalledWith(expect.objectContaining({
      max_tokens: 100,
      thinking: { type: "adaptive" },
      tools: [{ cache_control: undefined, description: undefined, input_schema: { type: "object" }, name: "weather" }],
    }));
  });
  ...
  it("normalizes native Messages streams", async () => {
    ...
    expect(output).toMatchObject([{ message: { id: "message-1" }, type: "message_start" }, { type: "message_stop" }]);
  });
  ...
  it("rejects Meta Messages fields that the route does not support", async () => {
    ...
    await expect(provider.messages({ ...common, topK: 10 })).rejects.toBeInstanceOf(UnsupportedParameterError);
