...
function fakeClient(methods: Record<string, unknown>): OtariClientLike {
  return methods as unknown as OtariClientLike;
}
    ...
    choices: [
      {
        ...
        message: {
          ...
          role: "assistant",
          toolCalls: [
            { function: { arguments: "{}", name: "weather" }, id: "call-1", type: "function" },
          ],
        },
      },
    ],
...
describe("Otari provider", () => {
  it("uses native fetch transport with hosted-platform authentication", async () => {
    const fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify(completionResponse()), {
      headers: { "Content-Type": "application/json" },
      status: 200,
    }));
    const provider = new OtariProvider({ clientOptions: { fetch, platformToken: "platform-token" } });
  ...
  it("covers the fetch transport across JSON, streams, forms, bytes, and batches", async () => {
    ...
    const fetch = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.endsWith("/chat/completions") && body.stream === true) {
        return new Response([
          ...
          "data:\n\n",
          `data: ${JSON.stringify({
            choices: [{ delta: { content: "streamed" }, finish_reason: "stop", index: 0 }],
            created: 1,
            id: "chunk-transport",
            model: "openai:model-a",
          })}\n\n`,
      ...
      if (url.endsWith("/responses")) return Response.json({ id: "response-transport", output: [] });
      ...
      if (url.endsWith("/rerank")) {
        return Response.json({ results: [{ index: 0, relevanceScore: 0.9 }] });
      }
      if (url.endsWith("/images/generations")) {
        return Response.json({ created: 1, data: [{ url: "https://example.com/image.png" }] });
      }
      ...
      if (url.endsWith("/audio/transcriptions")) {
        ...
        return transcriptionCalls === 1
          ...
          : new Response("text transcript", { headers: { "Content-Type": "text/plain" } });
      }
      if (url.endsWith("/models")) {
        return Response.json({ data: [{ created: 1, id: "model-a", ownedBy: "openai" }] });
      }
      ...
      if (url.includes("/results?")) {
        return Response.json({ results: [{ custom_id: "request-1", result: completionResponse() }] });
      }
    ...
    const chunks = [];
    for await (const chunk of completionStream as AsyncIterable<ChatCompletionChunk>) chunks.push(chunk);
    expect(chunks).toMatchObject([{ choices: [{ delta: { content: "streamed" } }] }]);

    await expect(provider.responses({ input: "hello", model: "openai:model-a" }))
    ...
    await expect(provider.rerank({ documents: ["a"], model: "cohere:rerank", query: "a" }))
      .resolves.toMatchObject({ results: [{ index: 0 }] });
    await expect(provider.imageGeneration({ model: "openai:image", prompt: "fox" }))
      .resolves.toMatchObject({ data: [{ url: "https://example.com/image.png" }] });
    await expect(provider.speech({ input: "hello", model: "openai:tts", voice: "alloy" }))
      .resolves.toEqual(new Uint8Array([1, 2, 3]));
    await expect(provider.transcription({
      file: new File(["audio"], "clip.wav", { type: "audio/wav" }),
      model: "openai:whisper",
      timestampGranularities: ["word", "segment"],
    })).resolves.toMatchObject({ text: "json transcript" });
    await expect(provider.transcription({ file: new Blob(["audio"]), model: "openai:whisper" }))
    ...
    await expect(provider.createBatch({ endpoint: "/v1/chat/completions", inputFilePath }))
      .resolves.toMatchObject({ id: "batch-transport" });
    await expect(provider.retrieveBatch("batch-transport", { provider: "openai" }))
      .resolves.toMatchObject({ id: "batch-transport" });
    await expect(provider.cancelBatch("batch-transport", { provider: "openai" }))
      .resolves.toMatchObject({ status: "cancelling" });
    await expect(provider.listBatches({
      after: "batch-before",
      limit: 2,
      providerOptions: { provider: "openai" },
    })).resolves.toHaveLength(1);
    await expect(provider.retrieveBatchResults("batch-transport", { provider: "openai" }))
      .resolves.toMatchObject({ results: [{ customId: "request-1" }] });

    expect(fetch).toHaveBeenCalledWith(
      expect.stringContaining("/chat/completions"),
      expect.objectContaining({
        headers: expect.objectContaining({ "Otari-Key": "Bearer gateway-key", "X-Test": "yes" }),
      }),
    );
  });

  it("normalizes fetch transport errors and requires an endpoint without a platform token", async () => {
    expect(() => new OtariProvider({ clientOptions: { fetch: vi.fn() } })).toThrow(/requires apiBase/u);
    const fetch = vi.fn().mockResolvedValue(new Response(
      JSON.stringify({ code: "rate_limit", message: "Slow down" }),
      { headers: { "Content-Type": "application/json", "Retry-After": "3" }, status: 429 },
    ));
    const provider = new OtariProvider({ apiBase: "https://otari.example", clientOptions: { fetch } });
    await expect(provider.completion({ messages: [], model: "openai:model-a" }))
  ...
  it("uses the Otari client contract and normalizes generated camel-case responses", async () => {
    ...
    await expect(provider.completion({
      ...
      choices: [{ finishReason: "tool_calls", message: { reasoning: "thinking", toolCalls: [{ id: "call-1" }] } }],
      provider: "otari",
      serviceTier: "default",
      systemFingerprint: "fingerprint",
      usage: { completionTokens: 2, promptTokens: 3, totalTokens: 5 },
  ...
  it("normalizes completion streams and provider errors", async () => {
    ...
    const stream = await provider.completion({ messages: [], model: "openai:model-a", stream: true });
  ...
  it("uses native Responses and Messages gateway routes", async () => {
    ...
    await expect(provider.responses({
      input: "hello",
      maxOutputTokens: 50,
      model: "openai:model-a",
    ...
    })).resolves.toMatchObject({ stopReason: "end_turn", usage: { inputTokens: 2 } });
  ...
  it("normalizes embeddings, models, images, audio, moderation, and reranking", async () => {
    const client = fakeClient({
      ...
      moderation: vi.fn().mockResolvedValue({
        id: "mod-1",
        model: "openai:moderation",
        results: [{ categories: { harassment: false }, categoryScores: { harassment: 0.1 }, flagged: false }],
      }),
    ...
    await expect(provider.embedding({ input: "hi", model: "openai:embedding" })).resolves.toMatchObject({
      provider: "otari",
      usage: { totalTokens: 1 },
    });
    await expect(provider.listModels()).resolves.toMatchObject([{ id: "model-a", ownedBy: "openai" }]);
    await expect(provider.imageGeneration({ model: "openai:image", prompt: "fox" })).resolves.toMatchObject({
      data: [{ b64Json: "image" }],
      provider: "otari",
    });
    await expect(provider.transcription({ file: new Blob(), model: "openai:whisper" })).resolves.toMatchObject({
      text: "transcribed",
    });
    await expect(provider.speech({ input: "hi", model: "openai:tts", voice: "alloy" })).resolves.toEqual(
      new Uint8Array([1, 2]),
    );
    await expect(provider.moderation({ input: "hi", model: "openai:moderation" })).resolves.toMatchObject({
  ...
  it("implements the direct Otari batch lifecycle from JSONL", async () => {
    ...
    const provider = new OtariProvider({ apiBase: "https://otari.example" }, client);
    const inputFilePath = fileURLToPath(new URL("./fixtures/batch.jsonl", import.meta.url));
    await expect(provider.createBatch({ endpoint: "/v1/chat/completions", inputFilePath })).resolves.toMatchObject({
      id: "batch-1",
      provider: "openai",
    ...
    expect(createBatch).toHaveBeenCalledWith(expect.objectContaining({
      model: "model-a",
      requests: [{ body: expect.objectContaining({ model: "model-a" }), custom_id: "request-1" }],
    }));
    await expect(provider.retrieveBatch("batch-1", { provider: "openai" })).resolves.toMatchObject({ id: "batch-1" });
    await expect(provider.cancelBatch("batch-1", { provider: "openai" })).resolves.toMatchObject({ status: "cancelling" });
    await expect(provider.listBatches({ limit: 10, providerOptions: { provider: "openai" } })).resolves.toHaveLength(1);
    await expect(provider.retrieveBatchResults("batch-1", { provider: "openai" })).resolves.toMatchObject({
  ...
  it("maps the SDK's incomplete-batch error", async () => {
    const provider = new OtariProvider(
      { apiBase: "https://otari.example" },
      fakeClient({ retrieveBatchResults: vi.fn().mockRejectedValue({ batchStatus: "in_progress" }) }),
    );
    await expect(provider.retrieveBatchResults("batch-1", { provider: "openai" })).rejects.toBeInstanceOf(
