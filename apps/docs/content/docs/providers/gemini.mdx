...
The adapter currently advertises completions, streaming, thinking, multimodal input, embeddings,
and model listing. The Google API also has batch functionality, but any-llm-ts does not yet expose a
provider-neutral batch API, so `metadata.capabilities.batch` remains `false`. Image generation,
moderation, speech, transcription, the OpenAI Responses API, and a raw native-messages method are
also outside this adapter's current public surface.
