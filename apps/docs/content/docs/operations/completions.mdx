...
Set `timeout` in seconds to bound an individual request. `serviceTier` forwards the provider's
service-tier selection when the underlying API supports it:
...
Adapters that cannot preserve per-request timeout semantics reject `timeout` instead of silently
ignoring it.
