...
The stateless moderation helper defaults to `openai` when `provider` is omitted. Moderation results
are left as `unknown` so provider-specific categories and scores are preserved.
