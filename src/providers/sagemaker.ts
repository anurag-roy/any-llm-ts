...
export type SageMakerRuntimeClientLike = Pick<SageMakerRuntimeClient, "send">;

type SageMakerBody = string | Uint8Array | { transformToString(): Promise<string> };
...
function createSageMakerClient(options: ProviderOptions): SageMakerRuntimeClient {
  const endpoint = options.apiBase ?? getEnvironmentVariable("SAGEMAKER_ENDPOINT_URL");
    ...
    return [
      {
        ...
        id: isString(call.id) ? call.id : `call_${unixTimestamp()}_${index}`,
        type: "function",
      },
    ];
  });
...
function normalizeCompletion(value: JsonValue | undefined, model: string): ChatCompletion {
  ...
  if (Array.isArray(response.choices)) {
    choices = response.choices.map((item, index) => {
      ...
      return {
        ...
        message: {
          ...
          ...includeWhen(!(calls === undefined), { toolCalls: calls }),
        },
      };
    });
  } else {
    ...
    if (content === undefined && response.outputs !== undefined) {
      content = Array.isArray(response.outputs) ? response.outputs[0] : response.outputs;
    }
  ...
  return {
    ...
    id: isString(response.id) ? response.id : `chatcmpl-${unixTimestamp()}`,
    model,
...
function normalizeChunk(
  ...
  if (isString(token.text)) content = token.text;
  ...
  } else if (isString(value.generated_text)) {
    ...
    reason = choice.finish_reason === undefined ? null : finishReason(choice.finish_reason);
  }
...
export class SageMakerProvider extends BaseProvider {
  ...
  override async completion(
    ...
    return this.execute(async () => {
      ...
      const value = parseJsonValue(
        JSON.parse(await bodyText(output.Body)),
        "SageMaker completion response",
      );
  ...
  override embedding(params: EmbeddingParams): Promise<EmbeddingResponse> {
    const inputs = isString(params.input)
      ? [params.input]
    ...
    return this.execute(async () => {
      ...
      for (const [index, input] of inputs.entries()) {
        ...
        const response = objectValue(
          parseJsonValue(JSON.parse(await bodyText(output.Body)), "SageMaker embedding response"),
        );
        ...
        const embedding =
          Array.isArray(rawEmbedding) && Array.isArray(rawEmbedding[0])
            ? rawEmbedding[0]
            : rawEmbedding;
        if (!Array.isArray(embedding) || !embedding.every((item) => isNumber(item))) {
